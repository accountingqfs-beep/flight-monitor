...
def track_api(pd,calls):
    if "_api_usage" not in pd:pd["_api_usage"]={}
    m=datetime.utcnow().strftime("%Y-%m")
    # Seed: if this month has no data yet, start at 49 (pre-existing usage)
    if m not in pd["_api_usage"] and m=="2026-03":pd["_api_usage"][m]=49
    pd["_api_usage"][m]=pd["_api_usage"].get(m,0)+calls
    return pd["_api_usage"][m]
...
def process_trip(trip,pd):
    ...
    track_api(pd,results["api_calls"])
